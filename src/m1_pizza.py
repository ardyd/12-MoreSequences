...
def run_test_draw_points_on_circle():
    ...
    circle = rg.Circle(rg.Point(150, 200), 90)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    p_list = generate_points_on_circle(circle, number_of_points)
    for k in range(number_of_points):
        circle1 = rg.Circle(p_list[k], 10)
        circle1.fill_color = color
        circle1.attach_to(window)
        p_list[k].attach_to(window)
    window.render()
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  12 slices, thin (thickness=2) blue lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    out_list = generate_points_on_circle(circle, number_of_slices)
    for k in range(number_of_slices):
        line = rg.Line(out_list[k], circle.center)
        line.color = color
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    p_list = generate_points_on_circle(circle, number_of_segments)
    for k in range(number_of_segments - 1):
        line = rg.Line(p_list[k], p_list[k+1])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
    line_last = rg.Line(p_list[0], p_list[number_of_segments - 1])
    line_last.thickness = thickness
    line_last.color = color
    line_last.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    fancy_polygon(window, circle, 20, 7, 'white', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    circle.attach_to(window)
    p_list = generate_points_on_circle(circle, number_of_lines)
    for k in range(number_of_lines):
        line = rg.Line(p_list[k % number_of_lines], p_list[(k + hops_to_next_point) % number_of_lines])
        line.thickness = thickness
        line.color = color
        line.arrow = 'last'
        line.attach_to(window)
    # line_last = rg.Line(p_list[0], p_list[number_of_lines - 1])
    # line_last.thickness = thickness
    # line_last.color = color
    # line_last.arrow = 'last'
    # line_last.attach_to(window)
    window.render()
